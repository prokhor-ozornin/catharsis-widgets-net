...
public sealed class IGoogleHtmlHelperExtensionsTests
{
  ...
  public void Analytics_Method()
  {
    ...
    Assert.Equal(new GoogleHtmlHelper().Analytics().ToHtml(), new GoogleHtmlHelper().Analytics(x => { }));
    Assert.Equal(new GoogleHtmlHelper().Analytics().Account("account").Domain("domain").ToHtml(), new GoogleHtmlHelper().Analytics(x => x.Account("account").Domain("domain")));
  }
  ...
  public void PlusOneButton_Method()
  {
    ...
    Assert.Equal(new GoogleHtmlHelper().PlusOneButton().ToHtml(), new GoogleHtmlHelper().PlusOneButton(x => { }));
    Assert.Equal(new GoogleHtmlHelper().PlusOneButton().Url("url").ToHtml(), new GoogleHtmlHelper().PlusOneButton(x => x.Url("url")));
  }
}